      ...
      <div className="w-full bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-xl flex flex-col">
        ...
        <div className="space-y-4 flex-1">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-6">
            <div className="flex flex-col bg-slate-900 rounded-lg p-1 gap-1 h-full">
              <button 
                onClick={() => {setMode('TEMPLATE'); setCalculationResult(null);}} 
                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all flex items-center justify-center ${mode === 'TEMPLATE' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Modo Padrão
              </button>
              <button 
                onClick={() => {setMode('MANUAL'); setCalculationResult(null);}} 
                className={`flex-1 py-2 text-sm font-bold rounded-md transition-all flex items-center justify-center ${mode === 'MANUAL' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'}`}
              >
                Modo Manual
              </button>
            </div>
            ...
            {mode === 'TEMPLATE' ? (
              ...
              <div className="bg-slate-900/80 border border-slate-700/50 rounded-lg p-3 flex flex-col justify-center relative overflow-hidden">
                ...
                <div className="flex gap-2 z-10">
                  ...
                  <button onClick={copyFromTemplate} className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 rounded-md text-xs font-bold transition-colors">Ir</button>
                </div>
              </div>
            )}
